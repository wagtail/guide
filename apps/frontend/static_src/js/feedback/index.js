...
export const handleFeedback = () => {
    ...
    const additionalFeedbackContainer = document.querySelector('[data-additional-feedback-container]')
    ...
    const post_feedback = async (feedback) => {
        ...
        try {
            ...
            if(feedback == "unhappy") {
                feedbackPk = data['pk']
                additionalFeedbackContainer.classList.remove("d-none")
            }
        } catch(err) {
            console.log(err)
        }
    ...
    if(submitButton) {
        submitButton.addEventListener("click", () => update_feedback(feedbackPk))
    }
}